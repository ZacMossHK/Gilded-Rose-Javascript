...
describe("UpdateItem class", () => {
  ...
  it("NormalItem: updates a normal item", () => {
    expect(updateItem.normalItem(new Item("foo", 0, 1))).toEqual(
      new Item("foo", 0, 0)
    );
  });

  it("normalItem: decrememnts quality x2 if sellIn <= 0", () => {
    expect(updateItem.normalItem(new Item("foo", -1, 0))).toEqual(
      new Item("foo", -1, -2)
    );
  });

  it("brie: updates brie", () => {
    expect(updateItem.agedBrie(new Item("Aged Brie", 0, 1))).toEqual(
      new Item("Aged Brie", 0, 2)
    );
  });

  it("brie: increments brie quality x2 if sellIn <= 0", () => {
    expect(updateItem.agedBrie(new Item("Aged Brie", -1, 0))).toEqual(
